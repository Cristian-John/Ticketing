    ...
    public attachListeners(container: HTMLElement) {
        ...
        if (prefsBtn) {
            prefsBtn.addEventListener('click', () => {
                // ModalsManager is available globally or we can import it, but to avoid circular dependencies 
                // if not imported, we can just dispatch to the global ModalsManager or call it directly.
                // Assuming ModalsManager is available via window or we can just find the element and add 'show'
